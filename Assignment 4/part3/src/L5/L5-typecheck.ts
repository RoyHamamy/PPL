...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         isDefineTypeExp, isTypeCaseExp, DefineTypeExp, TypeCaseExp, CaseExp, isAtomicExp, makeLitExp, VarDecl } from "./L5-ast";
...
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         ...
         isRecord, ProcTExp, makeUserDefinedNameTExp, Field, makeAnyTExp, isAnyTExp, isUserDefinedNameTExp, isAtomicTExp, isCompoundTExp, isNonTupleTExp, isField, makeNonEmptyTupleTExp, makeEmptyTupleTExp, makeSymbolTExp,makeRecord, makeLitTExp } from "./TExp";
...
const isSubType = (te1: TExp, te2: TExp, p: Program): boolean =>{
    // if(isAnyTExp(te2))
    //     return true;
    // else if(isUserDefinedTExp(te2) || isUserDefinedNameTExp(te2)) {   
    //     const parents = getParentsType(te1,p)
    //     for(const p in parents){
    //         if(isUserDefinedTExp(p) || isUserDefinedNameTExp(p)){
    //             if(te2.typeName === p.typeName)
    //                  return true; 
    //         }               
    //     }
    // }
    ...
    if(equals(te1,te2))
        return true;
const y=bind(checkCoverType([te1,te2],p),(x)=>makeOk(equals(x,te2)));
    if(isOk(y))
        return y.value;
...
export const initTEnv = (p: Program): TEnv =>{
    let arr = getDefinitions(p);
    let definitionsVar : TExp[] = map((y: DefineExp) => y.var.texp,arr);
    let definitionsVal : string[] = map((y: DefineExp) => y.var.var,arr);
    
    let arr2 = getTypeDefinitions(p);
    let defineTypeVar : string[] = map((y: UserDefinedTExp) => y.typeName + "?",arr2);
    let defineTypeVal : TExp[] = map((y: UserDefinedTExp) => makeProcTExp([makeAnyTExp()],makeBoolTExp()),arr2);
        
    let arr3 = getRecords(p);
    let recordVar : string[] = map((y: Record) => y.typeName + "?",arr3);
    let recordVal : TExp[] = map((y: Record) => makeProcTExp([makeAnyTExp()],makeBoolTExp()),arr3);    
    
    let recordVal2 : string[] = map((y: Record) => "make-" + y.typeName,arr3);
    let recordVar2 : TExp[] = map((y: Record) => 
    makeProcTExp(y.fields.map((x:Field)=>x.te),makeUserDefinedNameTExp(y.typeName)),arr3);

        

    let strings = definitionsVal.concat(defineTypeVar.concat(recordVar.concat(recordVal2)));
    let Texps = definitionsVar.concat(defineTypeVal.concat(recordVal.concat(recordVar2)));

    return makeExtendTEnv(strings, Texps, makeEmptyTEnv());
}
...
export const checkUserDefinedTypes = (p: Program): Result<true> =>{
    ...
    return (checkRecursive(p)&&checkSomeFields(p)) ? makeOk(true) : makeFailure("Fail");
...
export const checkSomeFields = (p: Program): Boolean =>{
    let records=getRecords(p);
    let nameEq =map((x:Record)=>records.filter((y:Record)=>y.typeName===x.typeName,records));
    let recoredEq= map((x:Record)=>records.filter((y:Record)=>y===x,records));
    return nameEq.length==recoredEq.length;
}



// export const checkRecursiveFields = (parent : UserDefinedTExp, r : Record) : Boolean => {
//     return r.fields.some((f : Field) => f.fieldName === parent.typeName);
// }

export const checkRecursive = (p: Program): boolean =>{
    const uds : UserDefinedTExp[] = getTypeDefinitions(p);
    let ans = map((y :UserDefinedTExp)=> checkUD(y) , uds);
    return all((x : boolean) => x , ans);
}

const checkUD = (ud : UserDefinedTExp) : boolean =>{
    const records : Record[] = ud.records;
    const ans : boolean[] = map((x :Record) => func(x, ud), records)
    const filtered :boolean[] = filter((x) => x === true, ans)
    // cons x = records.some()
    return filtered.length > 0;
}

const func = (r :Record, ud :UserDefinedTExp) : boolean => {
    const fields : Field[]  = r.fields;
    const ans : boolean[] = map((x:Field) => x.te === ud, fields)
    const filtered :boolean[] = filter((x) => x === true, ans)
    return filtered.length == 0
}
...
const checkTypeCase = (tc: TypeCaseExp, p: Program): Result<true> => 
    ...
    // (in any order)
    {
    const typeRes = getTypeByName(tc.typeName,p)
    if(!isOk(typeRes))return  makeFailure(typeRes.message)
    if(isUserDefinedTExp(typeRes.value)){
        if(!checkRecursive(p))return makeFailure("Error");
        if(!checkUD(typeRes.value))return makeFailure("Error2");        
    }
    return makeOk(true);
    }
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv, p: Program): Result<TExp> => {
    ...
    const constraint2 = bind(thenTE, (thenTE: TExp) =>
                            bind(altTE, (altTE: TExp) =>
                                checkEqualType(thenTE, altTE, ifExp, p) ? checkEqualType(thenTE, altTE, ifExp, p) : checkEqualType(altTE, thenTE, ifExp, p)));      
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv, p: Program): Result<VoidTExp> => {
    ...
    return bind(constraint, (te:TExp) => checkEqualType(te,texp,exp,p)? makeOk(makeVoidTExp()): makeFailure("Error"));
};
...
export const typeofLit = (exp: LitExp, _tenv: TEnv, _p: Program): Result<TExp> =>
    ...
    //     return makeOk(makeSymbolTExp(exp.val))
    // else if(isCompoundSExp(exp.val))
    //         return makeOk(makeEmptyTupleTExp())
    // else        
    //     return makeFailure("wrong type of Lit")
    makeOk(makeLitTExp());
...
export const typeofTypeCase = (exp: TypeCaseExp, tenv: TEnv, p: Program): Result<TExp> => {
    ...
    const every = exp.cases;

    let arr = [];
    
    for(let c of every){

        const res = getRecordByName(c.typeName , p);

        let v : string[] = [];
        
        for (let vardecs of c.varDecls){
            v.push(vardecs.var);
        }
        ...
        if(isOk(res))
        {
            ...
            for(let f of res.value.fields){
                tex.push(f.te);
            }

            const env : TEnv = makeExtendTEnv(v , tex , tenv);

            var ty = typeofExps(c.body,env,p);
    ...
    {
        const ans = checkCoverType(arr,p);
        return ans;
    }
