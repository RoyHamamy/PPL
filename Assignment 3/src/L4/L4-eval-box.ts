...
import { applyEnv, applyEnvBdg, globalEnvAddBinding, makeExtEnv, setFBinding,
            theGlobalEnv, Env, FBinding, extendFrame } from "./L4-env-box";
...
const evalTraceExp = (exp: TraceExp): Result<void> =>{
    const check = applyEnv(theGlobalEnv,exp.var.var);

     if(isOk(check)){
         
         if(isClosure(check.value)){
             const tracer = makeTracedClosure(check.value,exp.var.var);

             setBox(theGlobalEnv.frame, extendFrame(unbox(theGlobalEnv.frame), exp.var.var, tracer));
         }
        }
...
const applyTracedClosure = (proc: TracedClosure, args: Value[]): Result<Value> => {
    printPreTrace(proc.name, args, keep);
    
    const e = proc.closure;
    
    keep = keep + 1;

    const variables = map((vars: VarDecl) => vars.var, e.params);
    
    const ans =  evalSequence(e.body,makeExtEnv(variables, args, e.env));

    if(isOk(ans)){

        keep = keep - 1;

        printPostTrace(ans.value, keep)
    }
