...
(define pipe$ 
  (lambda(lst cont) 
    (if (empty? (cdr lst))(cont(car lst)) 
        (pipe$ (cdr lst) (lambda (res-cdr) (cont (compose$ (car lst) res-cdr id)))))))
              

;;; Q1.c

; Signature: reduce-prim$(reducer, init, lst, cont)
; Type: [[[T1 * T1] -> T1] * T1 * [T1,...,T1] * [T1 -> T1]] -> T1
...
(define reduce-user$ (lambda (reducer$ init lst cont)
  (if (empty? lst) (cont init) 
      (reducer$ init (car lst) (lambda (res) (reduce-user$ reducer$ res (cdr lst) cont))
      ))))
...
(define take-while
  (lambda (lz-lst f)
    (if (or (equal? (f (head lz-lst)) #f)(empty-lzl? lz-lst))
       empty-lzl
      (cons (head lz-lst)
            (take-while (tail lz-lst) f)))))
  ...
  (lambda (lz-lst f)
    (if (or (equal? (f (head lz-lst)) #f)(empty-lzl? lz-lst))
      ...
      (cons-lzl (head lz-lst)
            (lambda()(take-while-lzl (tail lz-lst) f))))))
...
; Purpose: Returns the reduced value of the given lazy list 
 (define reduce-lzl 
  (lambda (reducer init lz-lst)
    (if (empty-lzl? lz-lst)
       init
      (reduce-lzl reducer (reducer init (head lz-lst)) (tail lz-lst)))))
